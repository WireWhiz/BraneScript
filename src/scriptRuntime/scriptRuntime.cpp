...
namespace BraneScript
{
    ...
    struct AssemblyCtx
    {
        ...
        RT getReg(SerializedValue value)
        {
            ...
            {
                assert(value.storageType != ValueStorageType_DerefPtr);
                assert(value.storageType != ValueStorageType_Const);
                assert(value.index < registers.size());
    ...
    Script* ScriptRuntime::loadScript(IRScript* irScript)
    {
        assert(irScript);
        auto script = std::make_unique<Script>();
        ScriptAssembleContext assembleContext{irScript, script.get()};
        ...
        for(auto& s : irScript->linkedStructs)
        {
            ...
            assert(def);
            assembleContext.linkedStructs.push_back(def);
        }
        ...
        for(auto& func : irScript->linkedFunctions)
        {
            ...
            assembleContext.linkedFunctions.push_back(funcData);
        }
        ...
        for(auto& func : irScript->localFunctions)
        {
            ...
            FunctionData fPtr = loadFunction(&func, &assembleContext);

            script->functionNames.insert({func.sig, script->functions.size()});
            script->functions.push_back(fPtr.pointer);
        }
    ...
    FunctionData ScriptRuntime::loadFunction(IRFunction* function, ScriptAssembleContext* parentCtx)
    {
        ...
        if(parentCtx)
            ctx.currentScript = parentCtx->irScript;
        ...
        printf("Assembling function: %s %s\n", function->returnType.type.c_str(), ctx.currentFunction->sig.c_str());

        ctx.registers.resize(function->maxRegs);
        ctx.constants.resize(function->maxMemLocations);
        ...
        auto retType = strToASMType(function->returnType.type);
        sigBuilder.setRet(retType);
        auto* f = cc.addFunc(sigBuilder);

        if(parentCtx && parentCtx->script)
        {
            auto globalsPtr = parentCtx->script->globalVars.data();
        ...
        while(!ctx.endOfCode())
        {
            ...
            switch(op)
            {
                ...
                case LOADS:
                {
                    ...
                    parentCtx->script->constStrings.push_back(std::unique_ptr<std::string>(text));
                ...
                case CALL:
                {
                    ...
                    if(fIndex >= 0)
                    {
                        assert(parentCtx->irScript);
                        // Internal func call
                        auto& function = parentCtx->irScript->localFunctions[fIndex];
                        assert(fIndex < parentCtx->irScript->localFunctions.size());
                        ...
                        asmjit::InvokeNode* in;
                        assert(fIndex <= parentCtx->script->functions.size());
                        if(fIndex < parentCtx->script->functions.size())
                            cc.invoke(&in, parentCtx->script->functions[fIndex], sb);
                        else
                    ...
                    // External func call
                    fIndex = -1 - fIndex;
                    assert(fIndex < parentCtx->linkedFunctions.size());
                    auto function = parentCtx->linkedFunctions[fIndex];
        ...
        {
            _exportedFunctions.insert({fPtr, function->sig});
            _linker->addFunction(function->sig, function->returnType.type, function->arguments.size(), fPtr);
        }
    ...
    void ScriptRuntime::unloadFunction(void* func)
    {
        if(_exportedFunctions.contains(func))
        {
            _linker->removeFunction(_exportedFunctions.at(func));
            _exportedFunctions.erase(func);
        }

        auto err = _runtime.release(func);
        assert(err == asmjit::kErrorOk);
    }
} // namespace BraneScript