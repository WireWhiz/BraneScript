...
void AotConditionBase::jumpOnConditionFalse(AotValue& condition, uint32_t markIndex, CompilerCtx& ctx)
{
    switch(condition.valueIndex.storageType)
    {
        ...
        case ValueStorageType_AboveRes:
            ctx.function->appendCode(ScriptFunction::JBE, markIndex);
            break;
        case ValueStorageType_GreaterRes:
            ctx.function->appendCode(ScriptFunction::JLE, markIndex);
            break;
        case ValueStorageType_AboveEqualRes:
            ctx.function->appendCode(ScriptFunction::JB, markIndex);
            break;
        case ValueStorageType_GreaterEqualRes:
            ctx.function->appendCode(ScriptFunction::JL, markIndex);
            break;
    }
}

AotIf::AotIf(AotNode* condition, AotNode* operation) : _condition(condition), _operation(operation), AotConditionBase(nullptr, If)
{
...
AotWhile::AotWhile(AotNode* condition, AotNode* operation) : _condition(condition), _operation(operation), AotConditionBase(nullptr, If)
{
}
...
AotValue AotWhile::generateBytecode(CompilerCtx& ctx) const
{
    ...
    ctx.function->appendCode(ScriptFunction::MARK, beginMark);
    ...
    ctx.function->appendCode(ScriptFunction::JMP, beginMark);
    ctx.function->appendCode(ScriptFunction::MARK, exitMark);
    return {};
}
